...
GlobalMart, a leading e-commerce retailer with 300+ employees across 3 regions needs to implement a CI/CD pipeline for their e-commerce application. The purpose of the CI/CD pipeline is to streamline the deployment for their application and give the developers the ability to ship new features and innovate faster.
...
Compared to the manual setup mentioned above, there were some changes made when the infrastructure was defined using Terraform and are the following:

* Defined `S3 bucket` storage for `CodePipeline` artifact storage (manual step: S3 bucket automatically created as part of the CodePipeline)
* `CodeBuild` project was created separately (manual step: was automatically created as part of the CodePipeline)
    * This also allowed the `CodeBuild` output artifacts to be defined as "**/*" so that `CodeDeploy` has access to all the relevant build and config files, hence preventing the initial pipeline failure during `Deploy` stage (The manual setup didn't allow this option for the initial setup as AWS console allowed only defining `Input artifacts`)
* Created a `CodeStar Connection` (now AWS `CodeConnections`) instead of the default authentication via `OAuth` app (CodeStar Connection is recommended compared to OAuth as it uses dedicated app integrations instead of user tokens)

* Additional `IAM roles and policies` created for `CodePipeline` and `CodeBuild`
